...
`photos/dishes/` holds the 111 photographs as the shoot delivered them, and
`npm run photos` turns them into what the grid serves. A photograph can be
**held** — kept in the repository but placed on no card, when the dish in the
frame does not match the dish on the menu; `HELD` in the script names those and
why, and the run prints them. Nothing is held today. A source photograph that is
neither placed nor held stops the run, so a frame cannot be dropped in and
quietly forgotten.
...
The result is written as WebP at 340px and 540px: 111 photographs, 1.58 MB in
...
**16 of the 128 dishes have no photograph.** Those cards keep the photo well and
...
and only the café can resolve them, so the script reports them as outstanding
and exits non-zero. Second, does the site say what the page says? `en.ts` is
typed by hand and is checked against the English PDF dish by dish; `ar.ts` and
`ku.ts` are generated, so they are checked against a fresh parse, which catches
a stale file or a hand-edit.

A dish costing two different things in two languages is therefore expected and
is not a bug — it means two printed pages disagree, and the site reproduces each
language exactly as printed. What would be a bug is the site quoting a price
that is on none of the pages. The conflicts are written up in
[docs/menu-discrepancies.md](docs/menu-discrepancies.md) for the café to
resolve; §1 and §2 were once resolved by taking the Arabic figures and that
decision was then reversed, which is recorded there too.
